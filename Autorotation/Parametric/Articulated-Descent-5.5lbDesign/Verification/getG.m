function [loading,G,out] = getG(statesold,states,dt,dpsi,psi,model,rotor,physics,airfoil)
M = model.M;
Linv = model.Linv;
Nb = rotor.N_blades;
...
for k = 1:Nb %k: k-th blade; Nb: total # blades
    ...
    [eq34k(2*k-1:2*k,1),CTk,CLk,CMk,eq5,Q,alpha,M_flap,Fx1,Fz1] = exactBET(lambdak,betaveck,...
        betavecdotk,V_desc,omega,omegadot,r,dr,rotor,physics,airfoil);
...
eq123 = M*lambdastar + V_T*Linv*lambda - F;
...
function [eq34,CT,CL,CM,eq5,Q,alpha,M_flap,Fy1,Fz1] = exactBET(lambda,betavec,betavecdot,...
...
beta = betavec(1);
betadot = betavec(2);
theta = rotor.theta0*pi/180-beta*tan(delta3)+angle_tw*pi/180*r;
