...
dt = 0.05*[(logspace(-2,0,logsteps)),ones(1,tsteps-logsteps)];
...
%     rotor.chord = 0.025/.22*rotor.radius;
    rotor.delta3 = 30*pi/180;
    rotor.twist = 0;
    rotor.theta0 = [-pi/40*zeros(101,1);-pi/9*ones(100,1);pi/9*ones(200,1)];
else
...
rotor.mass = convmass(1.5,'lbm','kg');
rotor.Ib = 16.5*1e-3*rotor.radius^2/3;%0.000151;%4.5*%16.5
% rotor.Ib = 0.000151*.16/.22^5*rotor.radius^5;
...
model.body_dynamic_states = 2;
model.total_states = model.lambda_states+2*rotor.N_blades+model.body_dynamic_states;
%lambda0,beta1,betadot1,beta2,betadot2,omega,descent_speed,forward_speed
...
F = zeros(model.lambda_states+1,tsteps);%thrust, roll, pitch and yaw moments                                                          
psi = 0;
dpsi = 0;
