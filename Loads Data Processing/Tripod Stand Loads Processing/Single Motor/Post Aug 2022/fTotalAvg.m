...
for k = 1:length(StreamData.names)
    ...
    OMEGA = nanmean(StreamData.OMEGA{k}(2:end));
    ...
    cp_den = StreamData.rho{k} * (pi * StreamData.R^2) * (OMEGA*StreamData.R).^3;
    ...
    AvgData.avg_cps_est{k} = nanmean(RevData.avg_cps_est{k});
    ...
    cps_bias =  (2.4 * OMEGA) / cp_den / StreamData.sigma;
    I_bias = 1.5; %standard 1% error of 150 A sensor
    Kt = StreamData.Kt; %EMRAX Torque Constant (measured 12/16/2021)
    IQ_bias = sqrt(2/3)*I_bias; %error propagation of CR_150A current sensors
    cps_est_bias =  (IQ_bias * Kt / sqrt(2) * OMEGA) / cp_den / StreamData.sigma;
    ...
    AvgData.err_cps_inner{k} = 1.96* sqrt( std(RevData.ms_cps_inner{k})^2 + cps_bias^2 )...
        / sqrt(SortedData.nrevs{k});
    AvgData.err_cps_est{k} = 1.96* sqrt( std(RevData.ms_cps_est{k})^2 + cps_est_bias^2 )...
        / sqrt(SortedData.nrevs{k});
