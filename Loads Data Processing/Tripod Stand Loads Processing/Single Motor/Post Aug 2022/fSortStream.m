function [StreamData, SortedData] = fSortStream(StreamData, SR, GR)
%{
...
StreamData.Kt = 0.675; %Estimated torque constant based on 12/16/2021 tripod stand testing of Q-I Curve Fit
...
%         end
    ...
    StreamData.OMEGA{k} = SR./StreamData.binsize{k} * 2 * pi / GR; % Convert to ROTOR RPM
...
%                 StreamData.encoder{k}(count:count-1+b) =[360/b:360/b:360]; 
            ...
            if b > (2^Nbits-1)
                ...
                SortedData.curr1{k}(n,:) = interp1(az, StreamData.curr1{k}(count:dcm8:count-1+b)', SortedData.azimuth{k}, 'pchip');
                ...
                SortedData.curr1{k}(n,:) = interp1(az, StreamData.curr1{k}(count:count-1+b)', SortedData.azimuth{k}, 'pchip');
                
                SortedData.curr2{k}(n,:) = interp1(az, StreamData.curr2{k}(count:count-1+b)', SortedData.azimuth{k}, 'pchip');
    ...
    SortedData.cps_est{k} = SortedData.IQ{k} * StreamData.Kt / sqrt(2) * GR ./ StreamData.rho{k} / (pi * StreamData.R^2) ./ (OMEGA'*StreamData.R).^2 /StreamData.R / StreamData.sigma;
