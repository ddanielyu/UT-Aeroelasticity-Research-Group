...
% Made changes so you can visualize dual motor revolution data with index
% angle
...
%             fnos_pdcols = (MeanData.diffcols == pdcols);
    ...
    for ii = 1:nvars
        index = cell2mat(SortedData.index(fnos_p(:,ii))');   % [ (Nrevs x nfiles) x Naz ]
    ...
    in1 = gramm('x', xvar, 'y', index, 'color', rev_vec);
    in1.set_names('x', 'Azimuth, deg', 'y', 'Index $\phi$, deg', 'color', 'Rev.');
    in1.geom_line();
    in1.stat_summary('type', stat_type, 'geom', 'area');
    
    in1.axe_property('XGrid', 'on', 'YGrid', 'on', 'XLim', [0 360], 'XTick', 0:60:360, 'Box', 'on');
    in1.set_text_options('font','Times','base_size',14, 'label_scaling', 1.25);
    figure('Position',[111 240 696 557]);
    in1.draw();
